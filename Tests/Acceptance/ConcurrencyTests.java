...
public class ConcurrencyTests extends ProjectTest{
    ...
    public void simultaneouslyAddManager(){
        Thread thread1 = new Thread("User2") {
            public void run(){
                manager5 = defineStoreManager(store2Id, user5ManagerOfStore2ToBeRemoved, user4LoggedInId);
            }
        };

        Thread thread2 = new Thread("User4") {
            public void run(){
                manager6 = defineStoreManager(store2Id, user6OwnerOfStore2, user4LoggedInId);
            }
        };
